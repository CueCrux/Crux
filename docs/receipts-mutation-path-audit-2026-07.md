...
| 14 | Latest-only daemon control snapshot | `try_replace_latest_daemon_control` ([fact_store.rs](../crates/corecrux-memory/src/fact_store.rs)) | **(a) journaled control replacement** | Restricted to private repo registry/scan control entities. One replayable `ReplaceLatest` event names every displaced fact id; prior bytes remain in the journal until the already-receipted compaction path (#8), and an active legal hold blocks replacement. |
