...
4. **Coverage measures execution, not assertion strength.** Region coverage
   proves a line ran; it does not prove a test would catch a regression. The
   non-blocking nightly [`mutants.yml`](../.github/workflows/mutants.yml) runs
   `cargo-mutants` on the trust-core crates to surface assertion gaps that
