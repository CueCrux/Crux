...
corecruxctl receipts verify-stream-receipt record.json --keyring keyring.json \
  --expect-receipt-id <receipt id> --kind model_invocation
```
...
`local`) and the receipt id, and the signed `kind` is one of the four stream
kinds. Other receipt classes the daemon signs with the same key and tenant
(e.g. `usage_ping`) share the record shape and are rejected on `kind`.
`--expect-receipt-id` and `--kind`, when given, must match the signed body.

On success it prints, as JSON on stdout, the fields decoded from the signed
body: `kind`, `receipt_id`, `tenant_id` and, for `model_invocation`,
`invocation_id`, `provider`, `model_id`, `model_version`,
`provider_request_id`, `prompt_hash`, `retrieval_set_hash`, `output_hash`
(absent optional fields are omitted). Only these printed fields are
signature-verified. The record's top-level `kind` and the payload's own
`invocation_id`, `prompt_hash`, `retrieval_set_hash`, `output_hash`,
`session_id`, `stable_hash` and `injected_stable_hash` are unsigned copies:
read the command's output, not `payload.output_hash`.
