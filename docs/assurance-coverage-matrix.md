...
|---|---|---|---|---|---|---|
| **Cloud model call — Anthropic `/v1/messages`, OpenAI `/v1/chat/completions`·`/v1/responses`** | Cloud witness mode of `crux-llm-shim` (opt-in: `CRUX_CLOUD_WITNESS=1` + client `ANTHROPIC_BASE_URL`/`OPENAI_BASE_URL` → loopback listener) | Witness process, separate from the agent process; key is a `0600` file (same-UID readable — see class 2 honest limit). Session attribution is separately proof-gated by the shared `CRUX_CLOUD_WITNESS_SESSION_TOKEN`; without a matching `x-crux-witness-auth`, traffic still flows but `session_hint` is omitted | Witness Ed25519 key (`witness.key`, 0600); every request/response evidence record includes its own signed random 128-bit nonce | Agent/user unsets the base-URL env → calls stop appearing in the witness ledger (detectable absence); a **same-UID** compromise can read the witness key and session token and forge; callers without the session token can still use the listener, but cannot obtain session attribution | A request with digest X was sent to provider Y at time T through the witness and a response with digest Z (model/usage/end-state as recorded) came back — signed by *this witness key*. When a daemon-signed mediation observation exists, that daemon also accepted the signed nonce only once within its freshness window | That the attributed session is a unique person or process (it proves only possession of the listener's shared token); global uniqueness across daemon restarts or outside the replay-cache window; that the agent had no other channel; content semantics (digests only); that the host/user account was uncompromised. **Delivery note:** with `CORECRUXD_STREAM_RECEIPTS=1`, the daemon recognises the nested `{record,witness}` envelope, verifies the Ed25519 signature over recursively key-sorted canonical JSON of `record` against the embedded public key, and checks that `kid` is derived from that key. Before appending, it rejects records older than 10 minutes or more than 60 seconds in the future and deduplicates by signed nonce (falling back to `receipt_id` for legacy records) in a bounded, TTL-pruned in-memory cache. It then retains the exact metadata-only envelope in a daemon-signed mediation observation, which feeds incident reconstruction; the best-effort dataplane stream and entity projection are populated only when that dataplane is configured. If daemon delivery is unavailable, the shim still falls back to its local JSONL spool. The replay cache is process-local and is not durable across daemon restarts; daemon verification still does **not** independently enrol or pin a trusted witness identity, and the same-UID custody limit is unchanged |
