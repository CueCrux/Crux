...
### Name your sessions: `state.title` + `state.summary`

Two **conventional optional keys** inside `state` are read by the console (Work › Sessions
and Meters › Token Burn) and by any other session lens. Set them on every `save_session`:

| Key | Type | What it is |
| --- | --- | --- |
| `state.title` | string | A short human name for the session — "M21 console round 10", not "session-42". |
| `state.summary` | string | One paragraph on what happened and where the work stands. |

```json
{ "session_id": "console-m21",
  "state": { "title": "Console round 10 (M21)",
             "summary": "Accordion icons + graph LOD anchor fixed; cost bars next.",
             "decisions": ["LOD switch preserves the pointer anchor"] } }
```

Both are optional and the daemon never requires them. When `title` is absent the console
falls back to the first conventional state line (`decisions[0]` / `note` / `context_summary` /
`summary`) and then to a short id, and labels the row **untitled** — so an unnamed session is
visibly unnamed rather than silently anonymous. Everything else in `state` stays opaque and
is preserved verbatim.

### Session identity is stamped for you

`save_session` records the authenticated writer on the session itself (`actor`) — you do not
pass it. Resolution matches `store_fact`'s `actor`: the agent token-name, upgraded to a
passport_id when `CORECRUXD_AGENT_PASSPORTS=1`. A daemon with no `CRUX_AGENT_TOKENS`
configured has no identity to stamp, so `actor` is absent there; it is never guessed, and
there is no backfill for sessions written before the stamp existed.

Note that `actor` is **not** a plan link. `passport_id` / `execplan_slug` on a console session
row come from session *bindings* and live coord *intents*, both keyed by the sealed-plan
`session_id_hex` minted by `POST /session`. Agent `save_session` ids live in a disjoint key
space (`__agent_session::<agent>::<slug>`), so they never match those joins — announce with
`coord_announce` if you want your session to appear against an ExecPlan.
...
- **Watched repos + AST graph (M7–M8).** Register a repo with `register_repo` (or `POST /v1/repos`); list with `list_repos`. `tenant_id` is a consistency assertion against the authenticated MCP tenant. A local `root_path` additionally requires a global MCP operator context, must be a canonical descendant of `CORECRUXD_REPO_SCAN_ALLOWED_ROOTS`, and always queues `scan_mode="async"`; inline local scans are rejected. `clone_url` remains tenant-scoped and deferred. `CORECRUXD_WORKSPACE_PATH` is a separate operator self-scan and never expands repo authority. An empty allowlist disables local paths. Scans use bounded Rust and tree-sitter parsers, and persisted snapshots have a fixed 64 MiB ceiling. `CORECRUXD_REPO_WATCH=1` uses bounded 30-second secure content-digest polling and full replacement scans under the same containment/work limits. The legacy `CORECRUXD_CODEGRAPH_EDGES` / `CORECRUXD_CODEGRAPH_EXTERNAL` append-only emitter is fail-closed at startup until repo-owned bounded edge snapshots replace it; manual relation APIs and existing read surfaces remain available.
