...
|---|---|
| Release binaries (`corecruxd-*`, `crux-*`, `corecruxctl-*`) | cosign keyless signature (`.sig` + `.pem` per file) + SLSA v1.0 provenance |
...
| `multiple.intoto.jsonl` | SLSA provenance for all of the above, generated by the SLSA L3 builder |
...
`corecruxctl-<target>`, the SBOM and the manifest (same flags, different
filenames).
...
`multiple.intoto.jsonl` (a release asset) attests that each binary was built
from this repository at this tag by the SLSA L3 trusted builder:
