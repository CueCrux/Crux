...
below are the console's own: violet for memory, amber for retrieval, mint for receipts. No cloud
in this diagram; that's the point.

```mermaid
%%{init: {"theme":"base","themeVariables":{"fontFamily":"ui-monospace,SFMono-Regular,Menlo,Consolas,monospace","fontSize":"13px","primaryColor":"#0D1726","primaryTextColor":"#EAF0F7","primaryBorderColor":"#31445F","lineColor":"#5D7290","clusterBkg":"#0A111E","clusterBorder":"#22314A","titleColor":"#93A4B8","edgeLabelBackground":"#0A111E","tertiaryTextColor":"#93A4B8"}}}%%
flowchart TD
    agent(["your agent<br/>MCP / HTTP / gRPC"])
    mcp["crux-mcp<br/>100+ budgeted tools"]
    ctl["corecruxctl<br/>verify · replay · audit"]
    daemon["corecruxd<br/>one binary · :14800"]
    router["crux-router<br/>RCX capability tokens"]
    vault["vaultcrux-local<br/>tool & content boundary"]

    subgraph retplane["RETRIEVAL PLANE"]
        retrieval["corecrux-retrieval<br/>BM25 + graph fusion"]
        index["corecrux-index<br/>.ccxi indexes"]
        codemap["repo code maps<br/>AST · typed graph"]
    end

    subgraph memplane["MEMORY PLANE"]
        memory["corecrux-memory<br/>facts · sessions · handoffs"]
        projections["corecrux-projections<br/>derived state · work board"]
    end

    subgraph recplane["RECEIPT PLANE"]
        receipts["corecrux-receipts<br/>CROWN · Ed25519<br/>offline verify"]
    end

    subgraph spine["APPEND-ONLY SPINE"]
        storage["corecrux-storage<br/>shards · BLAKE3"]
        segment["corecrux-segment<br/>sealed segments"]
    end

    embeddings["embeddings<br/>optional · BYO"]
    ...
    receipts --> storage
    storage --> segment
    retrieval -. "dense lane" .-> embeddings

    classDef glass fill:#0D1726,stroke:#31445F,color:#EAF0F7
    classDef entry fill:#060A12,stroke:#6CA8FF,color:#EAF0F7,stroke-dasharray:4 3
    classDef hub fill:#0D1726,stroke:#6CA8FF,color:#EAF0F7,stroke-width:2.5px
    classDef mem fill:#0D1726,stroke:#7C6CF0,color:#EAF0F7
    classDef ret fill:#0D1726,stroke:#E8823C,color:#EAF0F7
    classDef rec fill:#0D1726,stroke:#3DDC97,color:#EAF0F7
    classDef dim fill:#0A111E,stroke:#22314A,color:#93A4B8

    class agent entry
    class daemon hub
    class mcp,ctl,router,vault glass
    class memory,projections mem
    class retrieval,index,codemap ret
    class receipts rec
    class storage,segment,embeddings dim
```
...
```mermaid
%%{init: {"theme":"base","themeVariables":{"fontFamily":"ui-monospace,SFMono-Regular,Menlo,Consolas,monospace","fontSize":"13px","primaryColor":"#0D1726","primaryTextColor":"#EAF0F7","primaryBorderColor":"#31445F","lineColor":"#5D7290","edgeLabelBackground":"#0A111E","tertiaryTextColor":"#93A4B8"}}}%%
flowchart LR
    engine["CruxEngine<br/>frontier ingest · extraction<br/>process → return → delete"]
    core["CoreCrux<br/>GPU database spine<br/>dense · graph · event lanes + AMR"]
    score["ScoreCrux<br/>benchmarks · evals"]
    wiki["WikiCrux · wiki.cuecrux.com<br/>public wiki + live code maps<br/>served by this daemon"]
    ...
    core --- crux
    score --- crux
    crux -- "dogfooding in public" --> wiki
    crux -- "every call rides a passport" --> rcx
    plan --- crux

    classDef glass fill:#0D1726,stroke:#31445F,color:#EAF0F7
    classDef hub fill:#060A12,stroke:#6CA8FF,color:#EAF0F7,stroke-width:2.5px
    classDef live fill:#0D1726,stroke:#3DDC97,color:#EAF0F7

    class engine,core,score,rcx,plan glass
    class wiki live
    class crux hub
```
