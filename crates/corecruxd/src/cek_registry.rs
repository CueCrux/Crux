...
//! daemon-root passport via the `corecrux-secrets` XChaCha20-Poly1305 envelope
//! — the same at-rest primitive the daemon already uses for integration
//! secrets — and persisted to `data_dir/cek_registry.jsonl`, replayed at
//! startup. Destroying a subject's CEK removes the wrapped key from the
//! registry; any fact payload sealed under it then becomes cryptographically
//! unrecoverable — that is the crypto-shred.
//!
...
use corecrux_secrets::EncryptedEnvelope;
...
impl CekRegistry {
    ...
    pub fn get_or_create(&mut self, subject: &str) -> Result<[u8; 32], CekRegistryError> {
        ...
        let wrapped = corecrux_secrets::seal(&cek, &self.wrap_key);
        ...
        for (line_no, line) in BufReader::new(file).lines().enumerate() {
            ...
            match serde_json::from_str::<CekRecordV1>(&line) {
                Ok(CekRecordV1::Create {
                    ...
                    match corecrux_secrets::open(&wrapped_cek, &self.wrap_key) {
