...
// --- M3: macaroon caveat enforcement at the sync boundary -------------------

fn sync_caveat_enforce_app(trust_root: &SigningKey) -> Router {
    let mut state = test_app_state_with_auth(16, AuthMode::DevScopes);
    state.sync_mutual_auth = true;
    state.sync_peer_trust_root = Some(trust_root.verifying_key().to_bytes().to_vec());
    state.sync_caveat_enforce = true;
    router_with_route_auth(state, test_case_store(), RouteAuthMode::Enforce)
}
...
/// GET the tenant manifest presenting `token` (holder = `subject`) and return
/// the (status, body). Every caveat test drives this one read endpoint, which
/// routes through `require_sync_peer(.., "facts:read")`.
async fn sync_peer_manifest(
    app: Router,
    token: &rcx_capability_token::RcxCapabilityToken,
    subject: &SigningKey,
    tenant: &str,
    ...
    let response = app
        .oneshot(sync_http_request(
            "GET",
            &format!("/v1/sync/tenants/{tenant}/manifest"),
            sync_peer_headers(token, subject, subject, &nonce),
        ))
        .await
        .expect("manifest response");
...
#[tokio::test]
async fn sync_caveat_enforce_accepts_attenuated_token_within_narrowed_authority() {
    let (trust_root, subject, token) = sync_test_peer("tenant-acme");
    let attenuated = token
        .attenuate(
            rcx_capability_token::Caveat::TenantIdEq {
                tenant_id: "tenant-acme".to_string(),
            },
            &subject,
        )
        .attenuate(
            rcx_capability_token::Caveat::ExpiresAtLe {
                expires_at: unix_now_secs() + 1_800,
            },
            &subject,
        );
    let (status, _) = sync_peer_manifest(
        sync_caveat_enforce_app(&trust_root),
        &attenuated,
        &subject,
        "tenant-acme",
    )
...
#[tokio::test]
async fn sync_caveat_enforce_denies_request_outside_caveat_tenant() {
    // Base grant is tenant-acme and the request is tenant-acme (base check passes),
    // but the holder narrowed to a different tenant — effective authority is empty,
    // so the caveat denies the request.
    let (trust_root, subject, token) = sync_test_peer("tenant-acme");
    let attenuated = token.attenuate(
        rcx_capability_token::Caveat::TenantIdEq {
            tenant_id: "tenant-elsewhere".to_string(),
        },
        &subject,
    );
    let (status, body) = sync_peer_manifest(
        sync_caveat_enforce_app(&trust_root),
        &attenuated,
        &subject,
        "tenant-acme",
    ...
    assert_eq!(body["reason_class"], "peer_caveat_denied");
}

#[tokio::test]
async fn sync_caveat_enforce_rejects_expired_caveat() {
    // The base token is still valid (expires in ~1h) but an ExpiresAtLe caveat
    // shrinks the effective expiry into the past.
    let (trust_root, subject, token) = sync_test_peer("tenant-acme");
    let attenuated = token.attenuate(
        rcx_capability_token::Caveat::ExpiresAtLe {
...
#[tokio::test]
async fn sync_caveat_enforce_rejects_scope_subset_excluding_the_request() {
    // A scope-subset caveat that omits facts:read must deny a manifest read even
    // though the base grant and possession are valid (the handshake itself does
    // not check capability — the caveat does).
    let (trust_root, subject, token) = sync_test_peer("tenant-acme");
    let attenuated = token.attenuate(
        rcx_capability_token::Caveat::ScopeSubset {
...
#[tokio::test]
async fn sync_caveat_enforce_off_rejects_attenuated_token_fail_closed() {
    // Flag OFF (default): an attenuated token is rejected outright rather than
    // accepted with its caveats ignored (never grant the broader base authority).
    let (trust_root, subject, token) = sync_test_peer("tenant-acme");
    let attenuated = token.attenuate(
        rcx_capability_token::Caveat::TenantIdEq {
            tenant_id: "tenant-acme".to_string(),
    ...
    let (status, body) =
        sync_peer_manifest(sync_mutual_auth_app(&trust_root), &attenuated, &subject, "tenant-acme").await;
    assert_eq!(status, StatusCode::UNAUTHORIZED);
    assert_eq!(body["reason_class"], "peer_caveat_enforcement_disabled");
}

#[tokio::test]
async fn sync_caveat_enforce_rejects_stripped_caveat_signature() {
    // Present the caveats without the holder binding signature — the un-narrowing
    // attempt must be rejected as an unauthenticated caveat set.
    let (trust_root, subject, token) = sync_test_peer("tenant-acme");
    let mut tampered = token.attenuate(
        rcx_capability_token::Caveat::TenantIdEq {
            tenant_id: "tenant-acme".to_string(),
        },
        &subject,
    );
    tampered.caveat_signature = None;
    let (status, body) =
        sync_peer_manifest(sync_caveat_enforce_app(&trust_root), &tampered, &subject, "tenant-acme").await;
    assert_eq!(status, StatusCode::UNAUTHORIZED);
    assert_eq!(body["reason_class"], "peer_caveat_signature_rejected");
}

#[tokio::test]
async fn sync_caveat_enforce_on_leaves_unattenuated_token_unchanged() {
    // With enforcement ON, a token carrying no caveats behaves exactly as today.
    let (trust_root, subject, token) = sync_test_peer("tenant-acme");
    let (status, _) = sync_peer_manifest(sync_caveat_enforce_app(&trust_root), &token, &subject, "tenant-acme").await;
    assert_eq!(status, StatusCode::OK);
}
