...
pub fn tool_output_docs() -> Value {
    ...
    let mut rows = match json!([
        ...
        { "tool": "register_repo",            "output": "Authenticated-tenant POST /v1/repos: { repo, job_id?, note? }. clone_url registration defers cloning. A local root_path requires global MCP operator authority, must be allowlisted, and defaults to scan_mode=async; poll the returned job_id until succeeded/failed." },
