...
pub fn tool_output_docs() -> Value {
    ...
    let mut rows = match json!([
        ...
        { "tool": "code_liveness",            "output": "Proxied GET /v1/code-intel/liveness: { symbol, executed, executions, total_ns, executions_empty, executions_outcome_unrecorded, always_empty, exists_statically, flagged_dead_static, verdict, window }. `executed: false` means 'not seen in this window', never 'dead' — read `window` before concluding anything. `always_empty: true` is the opposite failure: it ran every time and returned nothing every time, which no error or exit code reports. It is only ever true when every execution declared an outcome, so read `executions_outcome_unrecorded` before treating a `false` as reassurance — undeclared sites report nothing at all." },
