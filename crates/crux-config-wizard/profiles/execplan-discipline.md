...
- A milestone is **not done** until it is (a) committed, or explicitly marked
  ...
  — every target must at least **compile** (Rust: `cargo check --workspace
  --all-targets`), plus the repo's own lint gate as CI runs it. A per-package
  check passes while a downstream *test* target is broken. Do not assume
  `clippy --all-targets` is the gate: a crate that denies `unwrap_used` /
  `expect_used` in production code while its own tests use them freely will
  report thousands of pre-existing errors under it, which is a deliberate
  posture and not a finding. Compilation of all targets is what catches the
  missing-struct-field class. The `gate:M<n>` fact's `tests_passing` field
  records the **workspace** result.
