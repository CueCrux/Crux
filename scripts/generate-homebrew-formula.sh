...
sha_for() {
  ...
  awk -v f="$2" '$2 == f || $2 == "*"f {print $1; found=1} END {exit !found}' \
    "${WORK}/${manifest}" \
...
sed \
  ...
  -e "s|{{SHA256_CRUX_DARWIN_ARM64}}|$(sha_for darwin-arm64 "crux-darwin-arm64")|" \
  -e "s|{{SHA256_CRUX_DARWIN_AMD64}}|$(sha_for darwin-amd64 "crux-darwin-amd64")|" \
  -e "s|{{SHA256_CRUX_LINUX_AMD64}}|$(sha_for linux-amd64 "crux-linux-amd64")|" \
  -e "s|{{SHA256_CTL_DARWIN_ARM64}}|$(sha_for darwin-arm64 "corecruxctl-darwin-arm64")|" \
  -e "s|{{SHA256_CTL_DARWIN_AMD64}}|$(sha_for darwin-amd64 "corecruxctl-darwin-amd64")|" \
  -e "s|{{SHA256_CTL_LINUX_AMD64}}|$(sha_for linux-amd64 "corecruxctl-linux-amd64")|" \
  "$TEMPLATE"
