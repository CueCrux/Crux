...
- **The observation aggregate feed no longer shows other passports' sessions.**
  `GET /v1/observations/aggregate` without `session_id` read every session's
  observation journal, so any `query:read` caller received every passport's
  prompts, responses and mediation receipts. The feed now carries only the
  caller's own sessions, matched on each record's scoped session id. An unbound
  operator (raw `admin:read`, no passport) keeps the full cross-session view.
  **Behaviour change** for passport-bound callers. (QA audit M2, B3)
- **Concurrent observe step opens no longer share a `seq`.** `POST
  /v1/observe/sessions/{id}/steps` minted `seq` under a read lock and stored
  the node under a later write lock, so two racing opens both took `max + 1`
  and the second overwrote the first's default `node_id`, dropping a step from
  the audit chain. The seq is now minted under the write lock that stores the
  node. (QA audit M7)
...
- **Memory parity.** Recall applies a match floor and reports honest misses
  instead of padding weak hits (#791); a curated engram tier plus a memory
  digest is rendered into `CLAUDE.md` and `AGENTS.md`, with engrams on by
  default (#792); read-only native memory ingest as a projection (#787).
- **Prompt-cache awareness.** A cache-invalidation ledger (#788), a monotone
  MCP tool surface per session so the tool list stops churning the cache
  (#789), and cold-context awareness (#790). The boot banner's Python renderer
  gained the cold-cache line and a token fallback (#793).
- **Proof-carrying adaptive packs, first steps.** The `pack.conformance.v1`
  manifest format (#767) and shadow-corpus staging with a replay harness
  (#768, #769).
- **Extension lifecycle seams.** Staged activation (#763), a conformance hook
  (#764), per-pack outcome events (#765), and atomic rollback to a pinned prior
  build (#766).
- **Delegated embedding has one payer per daemon** (#760).
- An `escrow_recovery_code` fuzz target (#758) and an enforced-by lint for
  load-bearing auth claims (#748).

### Changed

- Four Pro claims that had no gate in the daemon were withdrawn from sale
  rather than advertised (#761).

### Security

- Unwedged the security gates: yanked crates replaced and two live advisories
  resolved (#785). A failing scheduled security audit now notifies someone
  instead of failing silently (#786).
- Dependency bumps: blake3, syn, wat, crc32fast, uuid, and CI actions
...
- **One tenant erasure could wedge every write on the shard.** `forget-tenants`
  ...
  whose `run_id` equals its `segment_seq`. That pairing is not reliable:
  `publish_dir_run_v1` resolves a run-id collision by incrementing `run_id`, so
  a run describing segment *N* can be published as `run_id N+1` — and a stale
  `directory/` file is enough to trigger it (host `crux` carries 1083 dirrun
  files against 1061 segments). The displaced run survives the reclaim still
  naming a segment that is gone.

  `rebuild_directory_from_runs` treated that extent as corruption and returned
  `dirrun references missing segment_seq N`, which fails `ShardStorage::open` —
  and open is on the path of every write, while reads scan `segments/` directly
  and stay green. The result is the silent-500 signature this store has now hit
  twice, once for 44 hours: all ingest returning 500 with `/readyz` healthy, and
  `corecruxctl storage repair-manifest` reporting the shard *healthy* because
  the segment is already out of the manifest and only the directory still names
  it.

  An extent naming a retired segment is now a **miss**, not an error: the
...
- **The macOS release build compiles again.** Provenance retention is
  deliberately disabled off Linux — it needs descriptor-anchored filesystem
  operations — and the two `cfg(not(target_os = "linux"))` stubs return early
  to do that. The early return left the rest of each function unreachable,
  which `-D unreachable-code` rejected, so the aarch64-apple-darwin job failed
  and v0.5.61 shipped without macOS binaries or provenance attestation.

  Nothing functional changed on any platform; the lint allowances are scoped to
  non-Linux, so the shipping Linux build keeps full strictness. macOS is only
  compiled on `v*` tags, which is why this reached a release before anyone saw
  it. (#722)
...
- **Lending an approver identity is now refused wherever the device's own human
  can be named.** `bind_passport` on a device grant binds the approving admin's
  passport — which is correct for your own second machine, and impersonation if
  an admin ticks it while approving a colleague's. No mechanism can tell those
  apart, so rather than policing the intent the need is removed: where the
  identity rail can name the device's human, lending is refused and that human
  must arrive as themselves.

  A stronger rung that is configured but *unusable* refuses too, matching the
  ladder's no-silent-downgrade rule — a misconfigured identity rail must
  surface, not become a licence to lend. Where no stronger rung exists, lending
  remains the only way to name anyone and stays available. (#705, accepted risk
  R3)


- **`/activate` no longer exists on daemons that cannot use it.** The
  device-approval page was served unconditionally, so a daemon with
  `CORECRUXD_DEVICE_GRANT_ENABLED` off still returned a fully-branded
  "Approve device login" form that could never approve anything.

  That is pure phishing surface with zero function: an attacker who obtains a
  `user_code` gets a genuine, correctly-branded, TLS-served page on the real
  domain to send a console admin to. Phishing is the first threat this rail's
  own module docs name, and their stated mitigation — the page shows the
  requesting client name — is weakest precisely when the page is legitimate,
  because it is. It now 404s with the flag named, matching the rest of the
  rail's disabled-⇒-404 idiom.

  This does not make the page non-public where the grant *is* enabled; that is
  a front-proxy allowlist decision, since approval itself is already
  `admin:write`-gated server-side. (#705)

### Added

- **The board can now digest itself.** The daemon already derived the expensive
  parts of ExecPlan-board analysis — dependency depth, cycles, and the
  orchestrator edges that point the wrong way — but nothing ever read them on a
  cadence, so with 200+ open plans the analysis only existed when a human
  thought to ask.

  A new background pass (`CORECRUXD_CRUCIBLE=1`, default off, daily) runs that
  analysis and appends one `__board_digest__::<run_id>` fact. It surfaces only:
  it never edits a plan, changes a work state, or resolves a finding. It is
  also fully deterministic and offline — no model call, no egress, no
  credentials — so the same binary runs the same pass on a laptop and on a
  server.

  When `CRUX_EXECPLANS_ROOT` is unset it declines to write rather than report
  an empty board, because a digest asserting nothing is open is
  indistinguishable from a digest of a board the daemon cannot see. (#715)

- **Daemon-side enrichment can now use Claude, OpenAI, or a local model.**
  A completion provider with four modes (`none`, `anthropic`, `openai`,
  `local`). `none` is the default and builds no request at all, so the
  local-first path never depends on a model being configured, reachable, or
  paid for; `local` targets an OpenAI-compatible server on the machine and
  needs no credential.

  Anthropic and OpenAI get separate adapters rather than one shim, because they
  are separate protocols — different path, different auth header, a mandatory
  `max_tokens`, a top-level system field, and reply text in a content array
  rather than a choices array. Routing one through the other silently returns
  empty text instead of failing. `local` reuses the OpenAI adapter because
  Ollama, llama.cpp and vLLM all serve that shape natively. (#715)

- **The number of distinct passports a work gate requires is now the operator's
  to set.** `CORECRUXD_WORK_GATE_APPROVERS` takes `1` or `2`, default `2` — the
  shipped behaviour, where a requester may not resolve their own gate.

  A hard ban does not produce separation of duties on a deployment with one
  human in it. It produces a second passport minted solely to approve one's own
  work — which passes the check while recording `distinct_passports` for what is
  really one person. That is a *worse* audit trail than admitting single-party
  approval, so the count is configurable and the receipt records the truth
  either way.
  ...
  two-party?" is exactly the question that needs tamper-evident material):

  ```
  work:<id>:<action>:<state>:approvers=<policy>:parties=<1|2>:sep=<evidence>
  ```

  `sep` is `distinct_passports` or `none:self_approved`. **`distinct_passports`
  means exactly that** — the daemon verifies two passports differ and cannot
  establish they belong to two people; the receipt never claims otherwise, and
  nothing in it says "four eyes". The `work_gate_resolution` capability also
  reports the policy, so a single-party deployment is visibly single-party.

  Anything unset, malformed, or out of range resolves to `2`: a typo must never
  silently relax a human-oversight boundary. (#705)
...
- **The daemon now declares whether a human can resolve a work gate on it, and
  by which evidence rung.** `work_gate_resolution` joins the runtime-capability
  descriptor on `/v1/version`, selected by a ladder over what the deployment is
  actually configured for — proxy-asserted identity header, device-grant
  vouching, or an unverified local daemon — strongest configured rung first.

  **A configured rung that cannot issue blocks rather than downgrading.** An
  identity rail with no passport-bound allowlist entry does not quietly fall
  through to the device grant: it reports `degraded` and names the fix. Without
  that rule a misconfiguration silently converts strong oversight into weak
  oversight while the receipt looks identical.

  The Gates page derives its Approve/Reject affordance from this declaration
  instead of hardcoding a flow, renders the daemon's `reason` verbatim when
  resolution is unavailable, and fails closed when the descriptor is absent — so
  a control that renders as actionable where the daemon would refuse is now
  structurally impossible rather than merely absent today. The console acquires
  no bearer token to do it: the client's standing invariant (same-origin
  credentials only) is preserved. (#705)
...
- **A local-only daemon's Gates page was still empty for any tenant but
  `default`.** The previous entry widened the pending-gate read for *verified*
  credentials and deliberately kept auth-off confined to `default`, preserving
  the audit property that an unauthenticated reader must name a tenant rather
  than enumerate the queue. That keyed the decision on **auth mode**, when the
  property it actually protects is **reachability** — so the one posture with
  no online auth by design, a loopback daemon with a single operator at the
  keyboard, was left with exactly the symptom the original report described:
  an empty queue, and (because the console builds its tenant picker from this
  same response) no tenant to switch to.

  The unverified case now widens to every tenant only when the daemon's own
  listener is loopback-only **and** the request arrived on a direct loopback
  socket carrying no `Forwarded` / `X-Forwarded-For` assertion. Both halves are
  required: a reverse proxy on the same host makes every remote client's peer
  address loopback, so peer-is-loopback alone would hand a remote caller local
  authority. Anything reachable — proxied, or a non-loopback listener — keeps
  the narrow `default` view, and naming a tenant works exactly as before.
  `direct_local` only ever widens the unverified case: it cannot grant a tenant
  a verified token lacks, and it cannot rescue a missing tenant claim. (#706)
...
[unreleased]: https://github.com/CueCrux/Crux/compare/v0.5.65...HEAD
[0.5.65]: https://github.com/CueCrux/Crux/compare/v0.5.64...v0.5.65
